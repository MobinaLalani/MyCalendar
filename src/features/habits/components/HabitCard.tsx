...
export default function HabitCard({ habit, isEditing, onEdit, onDelete }: Props) {
  ...
  return (
    ...
    >
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${isEditing ? "bg-black" : "bg-(--yellow)"}`} />
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
              ...
              {completedCount > 0 && (
                <span className={`text-xs font-semibold ${isEditing ? "text-black/70" : "text-amber-400"}`}>
                  🔥 {completedCount} بار
                </span>
              )}
            </div>
          ...
          {!confirmDelete ? (
            <div className="flex shrink-0 gap-1.5">
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold border transition-all duration-200 ${
                  isEditing
                    ...
                    : "border-black/30 bg-white/8 text-slate-300 hover:bg-white/15 hover:border-black/60"
                }`}
              >
                ویرایش
              </button>
              <button
                ...
                className="rounded-lg px-3 py-1.5 text-xs font-semibold border border-red-500/20 bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all duration-200"
              >
                حذف
              </button>
            </div>
            ...
            <div className="flex shrink-0 gap-1.5 items-center">
              ...
              <button
                ...
                className="rounded-lg px-2.5 py-1.5 text-xs font-bold border border-red-500 bg-red-500 text-white hover:bg-red-600 transition-all duration-200"
              >
                بله
              </button>
              <button
                ...
                className="rounded-lg px-2.5 py-1.5 text-xs border border-white/10 bg-white/8 text-slate-300 hover:bg-white/15 transition-all duration-200"
              >
                نه
              </button>
            </div>
